...
const fetchMedicalRecord = async () => {
  try {
    const response = await api.get(`/api/medical_records/${recordId}`);
...
const submitForm = async () => {
  try {
    await api.put(`/api/medical_records/${recordId}`, {
    ...
    router.push('/medical_records'); // Redirect menggunakan Vue Router
  } catch (error) {
...
<template>
  <div class="container mt-5">
    ...
    <form @submit.prevent="submitForm">
      ...
      <button type="submit" class="btn btn-primary btn-block">Update Medical Record</button>
    </form>
  </div>
</template>
